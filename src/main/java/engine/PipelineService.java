...
public class PipelineService {
    ...
    public PipelineService(String[] args){
        PipelineOptions options = PipelineOptionsFactory.fromArgs(args).create();
        pipeline = Pipeline.create(options);
    }

    /**
     * Metoda uruchamia pipeline
     */
    public void run(){
        pipeline.run();
    }
     ...
     */
    public  PCollection<KV<String, String>> loadFiles(String path){
        return pipeline.apply("MatchFiles", FileIO.match().filepattern(path))
     ...
     */
    public  PCollectionView<List<String>> loadStopWords(String path){
        PCollection<String> stopWords = this.pipeline.apply("LoadStopWords", TextIO.read().from(path))
     ...
     */
    public  PCollection<KV<String, String>> emitLines(PCollection<KV<String, String>> texts){
     ...
     */
    public  PCollection<KV<String, String>> tokenize(PCollection<KV<String, String>> pathWithLines){
     ...
     */
    public  PCollection<KV<String, String>> removeStopWords(PCollection<KV<String, String>> tokens, PCollectionView<List<String>> stopWordsView){
     ...
     */
    public  PCollection<KV<String, Iterable<KV<String, Double>>>> tfidf(PCollection<KV<String, String>> filtered){
        ...
        PCollection<KV<String, Long>> df = termDoc.apply("DFPerTerm", Count.perKey());

        PCollection<Long> docCount = filtered
                .apply("ExtractDocs", Keys.create())
                .apply("DistinctDocs", Distinct.create())
                .apply("CountDocs", Count.globally());

        PCollectionView<Map<String, Long>> dfView = df.apply(View.asMap());

        PCollectionView<Long> nView = docCount.apply(View.asSingleton());

        PCollectionView<Map<String, Double>> idf = df.apply("ComputeIDF", ParDo.of(new DoFn<KV<String,Long>, KV<String,Double>>() {
            @ProcessElement
            public void process(@Element KV<String,Long> kv,
                                OutputReceiver<KV<String,Double>> out,
                                ProcessContext ctx) {
                long dfVal = kv.getValue();
                long N = ctx.sideInput(nView);
                double idfVal = Math.log((double)N / (double)dfVal);
                out.output(KV.of(kv.getKey(), idfVal)); // term -> idf
            }
        }).withSideInputs(nView)).apply(View.asMap());

        PCollection<KV<String, KV<String, Long>>> tfByTerm =
                tf.apply("RekeyTF",
                        ParDo.of(new DoFn<KV<String,Long>, KV<String,KV<String,Long>>>() {
                            @ProcessElement
                            public void process(@Element KV<String,Long> kv,
        ...
        PCollection<KV<String, Double>> tfidf =
                tfByTerm.apply("TFIDF",
                        ParDo.of(new DoFn<KV<String,KV<String,Long>>, KV<String,Double>>() {
                            @ProcessElement
                            public void process(@Element KV<String,KV<String,Long>> kv,
                                                OutputReceiver<KV<String,Double>> out,
                                                ProcessContext ctx) {
                                ...
                                Map<String, Double> idfMap = ctx.sideInput(idf); // alternatively precompute view
        ...
        PCollection<KV<String, KV<String, Double>>> byPath =
                tfidf.apply("RekeyToPath",
                        MapElements.into(
                                TypeDescriptors.kvs(
     ...
     */
    private PCollection<KV<String, Long>> tf(PCollection<KV<String, String>> filtered){
     ...
     */
    public  PCollection<FileDto> toDto(PCollection<KV<String, Iterable<KV<String, Double>>>> grouped){
